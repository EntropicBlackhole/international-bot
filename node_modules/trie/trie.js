...
var Trie = function () {
	...
	/**
	 * Get all matching words in a dictionary given a substring
	 */
	self.getMatchingWords = function (substring, caseSense = true) {
		if (typeof substring !== 'string' || !substring.length) { return false; }
		let words = [];
		for (let word of getWords(dictionary)) {
			if (caseSense) {
				if (word.includes(substring)) words.push(word)
			} else
				if (word.toLowerCase().includes(substring.toLowerCase())) words.push(word)
		}
		return words
	};
